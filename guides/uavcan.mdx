...
sidebarTitle: "UAVCAN"
description: "Run the Delta GaN ESC as a UAVCAN v0 (DroneCAN) node — CAN mode, node ID, ESC index, raw command modes, and telemetry."
icon: "network-wired"
keywords:
  - "UAVCAN"
  - "DroneCAN"
...
<Warning>
  **This is UAVCAN v0, now called DroneCAN.** It is _not_ UAVCAN v1 / Cyphal.
  ...
  The two are different, incompatible protocols that share a name. The Delta firmware implements v0 via `libcanard`, which is what PX4 and ArduPilot mean when they say "DroneCAN" or "UAVCAN" in their ESC configuration.
</Warning>
